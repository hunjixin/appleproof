// Generated from ONNX "../applemodel/apple_detector.onnx" by burn-import
...
static EMBEDDED_STATES: &[u8] =
    include_bytes!("/Users/waylon/code/appleproof/lib/src/model/apple_detector.bin");
...
impl<B: Backend> Model<B> {
    ...
    pub fn new(device: &B::Device) -> Self {
        ...
        let maxpool2d2 = MaxPool2dConfig::new([2, 2])
            ...
            .with_dilation([1, 1])
            .init();
        let averagepool2d1 = AvgPool2dConfig::new([8, 8])
            .with_strides([8, 8])
            .with_padding(PaddingConfig2d::Valid)
            .with_count_include_pad(false)
            .init();
        let gemm1 = LinearConfig::new(512, 64).with_bias(true).init(device);
    ...
    pub fn forward(&self, input1: Tensor<B, 4>) -> Tensor<B, 2> {
        ...
        let flatten1_out1 = {
            let leading_dim = averagepool2d1_out1.shape().dims[..1]
                .iter()
                .product::<usize>() as i32;
            averagepool2d1_out1.reshape::<2, _>([leading_dim, -1])
        };
